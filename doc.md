...
*   **Python Backend & Analysis Module:** This consists of a set of Python scripts that provide advanced analytical capabilities.
    ...
    *   **ML Analysis Scripts (`gcode_analyzer.py`, `train_model.py`):** These scripts are executed by Node-RED. `gcode_analyzer.py` is used to parse G-code files to extract metadata and generate thumbnails. `train_model.py` is used to train the machine learning models for power prediction. The `Live Predictor` flow in Node-RED uses the trained model to make real-time predictions.
...
*   `public.environment_data`
    *   **Purpose:** Stores environmental data from sensors.
    *   **Key Columns:**
        *   `timestamp` (Primary Key, Hypertable Time Column): The time of the reading.
        *   `device_id`: The sensor hub that generated the reading (e.g., `ESP32_SensorHub_Raptor`).
        *   `temperature_c`, `humidity_pct`: Ambient temperature and humidity.
...
*   **`GET /api/dpp_summary`**
    ...
    *   **Purpose:** Provides a comprehensive, real-time summary of all printers for the Digital Product Passport (DPP) frontend. This is the primary data source for the main carousel view.
    *   **Method:** `GET`
    *   **Query Parameters:**
    ...
    *   **Response Body:** A JSON object containing two main keys:
        *   `printers`: An array of objects, where each object represents a single printer and contains detailed information about its status, job progress, energy consumption, and the "smart tip" text.
        *   `globalHistory`: An array of objects representing the most recent print jobs completed across the entire fleet.
    *   **Example Usage:** The `dpp_page.html` frontend calls this endpoint on load and periodically to refresh the printer card display.
...
*   **Query for DPP Summary (from `dpp_simulator.py`)**
    *   **Purpose:** Gathers all necessary data for a single printer for the DPP card view. This includes current status, job progress, 24-hour energy consumption, and the last completed job's details.
...
2.  **Environment Configuration (Optional):**
    The default configuration is set in the `docker-compose.yml` file. For production deployments, it is recommended to create a `.env` file in the project root to override default credentials.
    ```env
    # .env
    POSTGRES_DB=enms_db
